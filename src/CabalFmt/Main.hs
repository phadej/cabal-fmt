module CabalFmt.Main (mainWithArgs) where
...
mainWithArgs args = do
    (opts', filepaths) <- O.handleParseResult (O.execParserPure O.defaultPrefs optsP' args)
    let opts = runOptionsMorphism opts' defaultOptions

    notFormatted <- catMaybes <$> case filepaths of
        []    -> fmap pure $ BS.getContents >>= mainWithOpts opts Nothing
        (_:_) -> for filepaths $ \filepath -> do
            contents <- BS.readFile filepath
            mainWithOpts opts (Just filepath) contents
...
mainWithOpts :: Options -> Maybe FilePath -> BS.ByteString -> IO (Maybe FilePath)
mainWithOpts opts mfilepath input = do
    -- name of the input
